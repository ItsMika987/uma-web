...
export const scoutedSupports = derived(supportRoster, ($r) =>
    supportCards.filter(c => $r.includes(c.id)).sort((a, b) => b.rarity - a.rarity || a.name.localeCompare(b.name))
);
...
export function randomSupportScout(): SupportCard {
    ...
    const pool = supportCards.filter(c => c.rarity === rarity);
    const source = pool.length > 0 ? pool : supportCards.filter(c => c.rarity === 1);
